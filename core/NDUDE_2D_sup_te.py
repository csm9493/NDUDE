...
class Test_NDUDE_2D_sup:
    ...
    def __init__(self, case = None, delta=0.05, model_delta = None, k = 3, test_data = 'BSD20', ep = 15, is_blind = False):
        ...
        if is_blind == False:
            if test_data == 'BSD20':
                self.save_file_name = 'NDUDE_2D_sup_test_result_BSD20_k'+str(k)+'_delta'+str(int(self.delta*100))
            elif test_data == 'Set13_256':
                self.save_file_name = 'NDUDE_2D_sup_test_result_Set13_256_k'+str(k)+'_delta'+str(int(self.delta*100))
            else:
                self.save_file_name = 'NDUDE_2D_sup_test_result_Set13_512_k'+str(k)+'_delta'+str(int(self.delta*100))
        else:
            if test_data == 'BSD20':
                self.save_file_name = 'NDUDE_2D_blind_sup_test_result_BSD20_k'+str(k)+'_delta'+str(int(self.delta*100))
            elif test_data == 'Set13_256':
                self.save_file_name = 'NDUDE_2D_blind_sup_test_result_Set13_256_k'+str(k)+'_delta'+str(int(self.delta*100))
            else:
                self.save_file_name = 'NDUDE_2D_blind_sup_test_result_Set13_512_k'+str(k)+'_delta'+str(int(self.delta*100))
    ...
    def make_model(self, ep):
        ...
        if self.is_blind == False:
            ...
            model_file_name = 'NDUDE_2D_sup_training_data_k'+str(self.k)+'_delta'+str(int(model_delta*100))+'_ep'+str(ep).zfill(2)+'.hdf5'
            print (model_file_name)
        else:
            model_file_name = 'NDUDE_2D_blind_sup_training_data_k'+str(self.k)+'_ep'+str(ep).zfill(2)+'.hdf5'
            print (model_file_name)
    ...
    def test_model(self):
        ...
        for ep in range(self.ep):
            ...
            for img_idx in range(self.num_te_data):
                ...
                denoising_result = self.get_prediction_label(flatten_noisy, prediction_result)
